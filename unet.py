...
try:
    ...
    # 脚本方式运行的兜底导入（固定到本包目录与utils目录）
    from fp16_util import convert_module_to_f16, convert_module_to_f32
    from utils import utils as _utils_mod
    softmax_helper = _utils_mod.softmax_helper
    sigmoid_helper = _utils_mod.sigmoid_helper
    InitWeights_He = _utils_mod.InitWeights_He
    no_op = _utils_mod.no_op
    to_cuda = _utils_mod.to_cuda
    maybe_to_torch = _utils_mod.maybe_to_torch
    from nn import (
        checkpoint,
        conv_nd,
        linear,
...
class AttentionPool2d(nn.Module):
    ...
    This module is used in the context of radio-related operations, perhaps for processing radio signal features.
    """
...
class TimestepBlock(nn.Module):
    ...
    In radio applications, timestep embeddings might represent different time intervals of radio signals.
    """
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    In radio processing, this could be used to process radio signals at different time intervals.
    """
...
class Upsample(nn.Module):
    ...
    In radio signal processing, upsampling might be used to increase the resolution of radio signals.
...
class Downsample(nn.Module):
    ...
    In radio signal processing, downsampling might be used to reduce the resolution of radio signals.
...
class ResBlock(TimestepBlock):
    ...
    In radio applications, channel changes could represent different frequency bands of radio signals.
    ...
    def _forward(self, x, emb):
        if self.updown:
            ...
            h = self.in_layers(x)
        # Ensure embedding input has the same dtype as Linear weights to avoid AMP dtype mismatch
        try:
            target_dtype = self.emb_layers[1].weight.dtype
        except Exception:
            target_dtype = emb.dtype
        emb_in = emb.to(target_dtype)
        emb_out = self.emb_layers(emb_in).to(h.dtype)
...
class AttentionBlock(nn.Module):
    ...
    In radio signal processing, this could be used to focus on specific parts of radio signals.
...
class QKVAttentionLegacy(nn.Module):
    ...
    In radio applications, QKV attention could be used to analyze different aspects of radio signals.
    """
...
class FFParser(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        B, C, H, W = x.shape
        # 适配任意 H, W：根据 rfft2 的输出宽度 W//2+1 动态对齐权重
        w_fft = (W // 2) + 1
        expected_shape = (C, H, w_fft, 2)
        if tuple(self.complex_weight.shape) != expected_shape:
            self.complex_weight = nn.Parameter(
                torch.randn(*expected_shape, dtype=torch.float32) * 0.02
            )
...
class UNetModel_v1preview(nn.Module):
    ...
    In radio applications, this model could be used for tasks such as radio signal denoising or classification.
    ...
    def __init__(
        ...
        # Ensure time embedding layers match the computation dtype in AMP/FP16
        if use_fp16:
        ...
        if self.num_classes is not None:
            ...
            if use_fp16:
                self.label_emb = self.label_emb.half()
    ...
    def forward(self, x, timesteps, y=None):
        ...
        hs = []
        # Cast timestep embedding to the model compute dtype before MLP to avoid FP16/FP32 mismatch
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels).to(self.dtype))

        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y).to(emb.dtype)
        ...
        for ind, module in enumerate(self.input_blocks):
            ...
            h = module(h, emb)
            hs.append(h)
        uemb, cal = self.highway_forward(c, None)
        h = h + uemb
...
class UNetModel_newpreview(nn.Module):
    ...
    def highway_forward(self,x, hs = None):
        if hasattr(self, 'hwm'):
            ...
            # highway模块被禁用，返回零张量作为占位符
            batch_size = x.shape[0]
            dummy_anch = th.zeros(batch_size, 1, x.shape[2], x.shape[3], device=x.device, dtype=x.dtype)
            dummy_cal = th.zeros(batch_size, 1, x.shape[2], x.shape[3], device=x.device, dtype=x.dtype)
            return dummy_anch, dummy_cal
...
class EncoderUNetModel(nn.Module):
    ...
    def __init__(
        ...
        if pool == "adaptive":
            ...
            self.out = nn.Linear(ch, self.out_channels)
...
class RadioNetwork(NeuralNetwork):
    def __init__(self):
        super(NeuralNetwork, self).__init__()
...
class Generic_UNet(RadioNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        # 注册瓶颈嵌入投影层，避免在 forward 中动态创建未注册模块
        self.emb_proj = conv_nd(2, final_num_features, 512, 1)
